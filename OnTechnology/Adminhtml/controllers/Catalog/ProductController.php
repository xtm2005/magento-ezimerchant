...
class OnTechnology_Adminhtml_Catalog_ProductController extends Mage_Adminhtml_Catalog_ProductController
{
    /**
	...
	/**
     * add product to ezi action for ajax request
     *
     */
    public function addproducttoeziAction()
    {
			...
			$paramhash["ACTION"] = "UpdateProduct";
			$paramhash["PRODUCTCODE"] = $product->sku;
			$paramhash["PRODUCTNAME"] = $product->name;
			$paramhash["PRODUCTPRICE"] = $product->price;
			
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, "https://api.ezimerchant.com/".$merchantid."/products/");
			...
			$ordercontent = $xml->entry->children('http://api.ezimerchant.com/schemas/2009/');
			
			if($ordercontent->success == 1)
			echo "<br>".$product->name ." is added to the Ezi site.";
			else
			{
				echo "<br>Sorry, we are facing some problem now.<br>Please try later.";
			}

        $content = $this->getLayout()->createBlock('adminhtml/catalog');
        $this->getResponse()->setBody($content->toHtml());
    }
